...
export const SectorDetail = () => {
  ...
  const { sectorsData, refreshData, loading } = useData();
  const { user, token } = useAuth();

  const [isModalOpen, setIsModalOpen] = useState(false);
  const [newOffering, setNewOffering] = useState({ name: '', description: '' });
  ...
  const handleAddSectorService = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('http://localhost:3001/api/sector_services', {
        ...
        body: JSON.stringify({ ...newOffering, sector_id: sectorId })
      });
      setNewOffering({ name: '', description: '' });
  ...
  const handleDeleteSectorService = async (e: React.MouseEvent, id: number) => {
    e.preventDefault();
    e.stopPropagation();
    if (!window.confirm("Are you sure you want to delete this sector offering?")) return;
    try {
  ...
  const data = sectorsData.find(s => s.id === sectorId);
  ...
  return (
    <div className="pt-32 pb-20 min-h-screen">
      <div className="container mx-auto px-6 max-w-[1440px]">
        <Link to="/sectors" className="inline-flex items-center text-gray-900 dark:text-gray-200 hover:text-gray-900 dark:text-gray-200 mb-8 transition-colors font-bold text-sm tracking-wide">
        ...
        <ScrollReveal className="mb-16 bg-white dark:bg-[#0a0e14] rounded-3xl p-8 md:p-12 border border-black/5 dark:border-white/5 shadow-sm dark:shadow-none relative overflow-hidden">
          <div className="absolute top-0 right-0 p-16 opacity-10 dark:opacity-[0.07] pointer-events-none text-gray-900 dark:text-white">
            <data.icon className="w-64 h-64" />
          </div>
          <div className="relative z-10">
            <h1 className="text-4xl md:text-5xl font-bold mb-6 text-gray-900 dark:text-white">
              {data.name} <DecryptedText text="Security" animateOn="view" revealDirection="center" speed={100} maxIterations={20} parentClassName="text-transparent bg-clip-text bg-gradient-to-r from-gray-300 to-gray-500 inline-block" />
            </h1>
            <p className="text-xl text-gray-600 dark:text-gray-400 max-w-3xl leading-relaxed">
              {data.desc} Explore our specialized services tailored exactly to the demands of this sector below.
            </p>
          </div>
        </ScrollReveal>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {data.services.map((service, idx) => (
            <ScrollReveal
              ...
              className="h-full"
            >
              <Link to={`/sectors/${data.id}/${service.id}`} className="block h-full outline-none">
                <div className="relative bg-white dark:bg-[#0a0e14] border border-gray-200 dark:border-white/5 rounded-3xl overflow-hidden group h-full min-h-[16rem] shadow-md dark:shadow-sm hover:shadow-xl dark:hover:shadow-sm transition-all duration-300">
                  {user && (
                    <button 
                      onClick={(e) => handleDeleteSectorService(e, service.id)}
            ...
            <ScrollReveal delay={0.2} className="h-full">
              <button onClick={() => setIsModalOpen(true)} className="block w-full h-full outline-none text-left">
                <div className="relative bg-gray-50 dark:bg-white/5 border border-dashed border-gray-300 dark:border-white/20 rounded-3xl overflow-hidden hover:bg-gray-100 dark:hover:bg-white/10 hover:border-gray-400 dark:hover:border-white/40 transition-all duration-300 min-h-[16rem] flex flex-col items-center justify-center text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white cursor-pointer">
                  ...
                  <h3 className="text-lg font-bold">Add Offering</h3>
                  <p className="text-sm mt-2">Instantly add to {data.name}</p>
                </div>
              </button>
            </ScrollReveal>
          )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-[#0a0e14] border border-gray-200 dark:border-white/10 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden relative">
            <div className="p-6 border-b border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5 flex justify-between items-center">
              <div>
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">Add Sector Offering</h3>
                <p className="text-xs text-gray-500 mt-1 uppercase tracking-widest">{data.name}</p>
              </div>
            ...
            <form onSubmit={handleAddSectorService} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-bold text-gray-900 dark:text-gray-300 mb-2">Offering Name</label>
                <input required placeholder="e.g. PCI-DSS Audits" value={newOffering.name} onChange={e => setNewOffering({...newOffering, name: e.target.value})} className="w-full bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-3 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all" />
              </div>
              <div>
